        ...
        @else
            <div @if (FilamentView::hasSpaMode()) {{-- format-ignore-start --}}x-load="visible || event (ax-modal-opened)"
                ...
                <button x-ref="button" x-on:click="togglePanelVisibility()"
                    ...
                    {{ $getExtraTriggerAttributeBag()->class(['fi-fo-date-time-picker-trigger', 'w-full']) }}>
                ...
                <div x-ref="panel" x-cloak x-float.placement.bottom-start.offset.flip.shift="{ offset: 8 }" wire:ignore
                    wire:key="{{ $livewireKey }}.panel" @class([
                    ...
                    <div class="grid gap-y-3">
                        @if ($hasDate())
                            <div class="flex items-center justify-between fi-fo-date-time-picker-panel-header">
                                <select x-model="focusedMonth"
                                    class="p-0 text-sm font-medium bg-transparent border-none cursor-pointer grow text-gray-950 focus:ring-0 dark:bg-gray-900 dark:text-white">
                                ...
                                <input type="number" inputmode="numeric" x-model.debounce="focusedYear"
                                    class="w-16 p-0 text-sm text-right bg-transparent border-none text-gray-950 focus:ring-0 dark:text-white" />
                            </div>

                            <div class="grid grid-cols-7 gap-1 fi-fo-date-time-picker-calendar-header">
                                <template x-for="(day, index) in dayLabels" x-bind:key="index">
                                    <div x-text="day"
                                        class="text-xs font-medium text-center text-gray-500 dark:text-gray-400"></div>
                                </template>
                            </div>

                            <div role="grid"
                                class="grid grid-cols-[repeat(7,minmax(theme(spacing.7),1fr))] gap-1 fi-fo-date-time-picker-calendar">
                        ...
                        @if ($hasTime)
                            <div
                                class="fi-fo-date-time-picker-time-inputs flex items-center justify-center py-2 rounded-lg rtl:flex-row-reverse g-gray-50">
                                ...
                                <span
                                    class="fi-fo-date-time-picker-time-input-separator text-xl font-medium text-gray-500 dark:text-gray-400">
                                    :
                                </span>
                                ...
                                @if ($hasSeconds())
                                    <span
                                        class="fi-fo-date-time-picker-time-input-separator text-xl font-medium text-gray-700 bg-transparent dark:text-white">
                                        :
                                    </span>
